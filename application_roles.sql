-- Module 2 - Demo 4 (application roles)
...
-- Step 3 - before activating the application role, verify that you can SELECT from Sales.Customer
SELECT TOP (5) * FROM Sales.Customer;
GO

-- Step 4 - activate the application role the cookie is stored in a temporary table to persist it across batch seperators
...
-- Step 5 - demonstrate that, once the application role is active, the user cannot access objects to which the role has no permissions
-- (This command will generate an error)
SELECT TOP (5) * FROM Sales.Customer;
GO
-- However, the user has access to objects which the role has permission to access
SELECT TOP (5) * FROM Purchasing.ShipMethod;
GO

-- Step 6 - show that the user's identity is still accessible from SUSER_SNAME() and SUSER_NAME(), but that USER_NAME is the application role
SELECT	SUSER_SNAME() AS suser_sname, SUSER_NAME() AS suser_name, USER_NAME() AS user_name;
GO
